...
// Admin — all, filterable by status
...
// Public — submit review (saved as pending, fires emails)
export const submitTestimonial = async (req, res) => {
  try {
    ...
    try {
      ...
      sendTestimonialAdminNotification(testimonial, toEmails).catch(err => console.error('❌ Admin review email failed:', err?.message));
    } catch {}

    // Confirm to reviewer
    sendTestimonialReviewerConfirmation(testimonial).catch(err => console.error('❌ Reviewer confirm email failed:', err?.message));
...
// Admin — create directly as approved
